...
export default function Dashboard({token,setToken}){
  ...
  useEffect(()=>{ fetch("/tasks/available", {headers: {"Authorization":"Bearer "+token}}).then(r=>r.json()).then(setTasks).catch(()=>{}) },[token]);
  ...
  function claim(id){ fetch("/tasks/"+id+"/claim", {method:"POST", headers: {"Authorization":"Bearer "+token}}).then(()=>alert("claimed")).catch(()=>alert("fail")) }
}
